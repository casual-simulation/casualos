...
// typedoc-plugin/api.ts
var import_typedoc = __toModule(require("typedoc"));
var import_path = __toModule(require("path"));
var project;
function parseProject() {
  const app = new import_typedoc.default.Application();
  app.options.addReader(new import_typedoc.default.TSConfigReader());
  app.bootstrap({
    tsconfig: import_path.default.resolve(__dirname, "tsconfig.json"),
    entryPoints: [import_path.default.resolve(__dirname, "entry.ts")]
  });
  const project2 = app.convert();
  return { app, project: project2 };
}
function getProject() {
  if (!project) {
...
module.exports = function pluginTypedoc(context, options) {
  ...
  return {
    ...
    configureWebpack: function(config, isServer, utils, content) {
      let modules = {};
      for (let child of content.types) {
        let script = `module.exports = ${JSON.stringify(child.default)};`;
        modules[`node_modules/@api/${child.name}`] = script;
      }
...
function loadContent(options) {
  const { app, project: project2 } = getProject();
  if (!project2) {
    console.warn("[docusarus-plugin-typedoc] Unable to load TypeDoc project!");
  }
  let allUsedTypes = new Set();
  let typesWithPages = new Set();
  let pages = new Map();
  const getPage = (hash) => {
  ...
  walk(project2, (child, parent, key) => {
    if ("kind" in child) {
  ...
  for (let page of pages.values()) {
    page.contents = (0, import_lodash.sortBy)(page.contents, (c) => c.order, (c) => c.name);
  }
  let types = [];
  for (let child of project2.children) {
    types.push({
      name: child.name,
...
function getReflectionHash(reflection) {
...
function getReflectionOrder(reflection) {
...
function resolveTypeReferences(typeReferences) {
  let result = new Set();
  for (let ref of typeReferences) {
    ...
    if (ref.typeArguments && ref.typeArguments.length > 0) {
      for (let inner of resolveTypeReferences(ref.typeArguments.filter((t) => t.type === "reference"))) {
        result.add(inner);
      }
    }
  }
...
function walk(obj, callback, parent = null) {
  walkSingle(obj, (value, parent2, key) => {
    callback(value, parent2, key);
    walk(value, callback, value);
  });
}
function walkSingle(obj, callback, parent = null) {
  let type = "kind" in obj ? obj.kindString : obj.type;
  let keys = keysMap[type] || [];
  for (let key of keys) {
    let value = obj[key];
