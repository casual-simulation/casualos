...
export const DataTagLink = ({tag}) => (
  <a href={new URL('./tags.mdx', import.meta.url).href + '#' + tag.replace(/[\.\(\)\@\[\]]/g, '').toLowerCase()}>
    <NormalCode>#{tag}</NormalCode>
  </a>
);

export const ListenTagLink = ({tag}) => (
  <a href={new URL('./listen-tags.mdx', import.meta.url).href + '#' + tag.replace(/[\.\(\)\@\[\]]/g, '').toLowerCase()}>
    <NormalCode>{tag}</NormalCode>
  </a>
);

export const ActionLink = ({action, children}) => (
  <a href={new URL('./actions.mdx', import.meta.url).href + '#' + action.replace(/[\.\(\)\,\?]/g, '').replace(/\s/g, '-').toLowerCase()}>
  {children ? children : (
