...
When CasualOS runs across a vector tag, it parses the numbers and converts it into a [`Vector2`](./math/vectors.mdx#vector2) or [`Vector3`](./math/vectors.mdx#vector3) object depending on if it contains 2 or 3 numbers.
...
See the documentation on [Vectors](./math/vectors.mdx) for more information and examples for scripting.
...
When CasualOS runs across a rotation tag, it parses the numbers and converts it into a [`Rotation`](./math/rotations.mdx#rotation) object.
...
See the documentation on [Rotations](./math/rotations.mdx) for more information and examples for scripting.
...
Script tags that have one of the names listed [here](./listen-tags.mdx) can be executed directly by CasualOS.
...
Finally, you can check out the comprehensive [list of actions](./actions.mdx) that are supported in CasualOS.
...
![Image of bots in the systemPortal](/img/glossary/systemPortal_02.png)
