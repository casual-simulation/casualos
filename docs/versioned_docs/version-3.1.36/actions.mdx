...
```typescript title='Get the rotation of this bot in the #home dimension'
...
Performs the given action. This function can be used to perform actions that you have stored as data without having to find out which function to call.
You can find a list of action types [here](https://github.com/casual-simulation/casualos/blob/develop/src/aux-common/bots/BotEvents.ts#L40).

The **first parameter** is the action that should be performed.

#### Examples:

```typescript title='Perform a toast action'
action.perform({
    type: 'show_toast',
    message: 'Hello, world!',
    duration: 2000
...
```typescript title='Perform an add bot action'
action.perform({
    type: 'add_bot',
    id: 'bot_id',
    bot: {
...
The file can later be retrieved by using <ActionLink action='os.getFile(urlOrRecordFileResult, endpoint?)' />.
...
The downloaded bots will be stored in the Version 1 format of the <GlossaryRef term='aux'>AUX File Format</GlossaryRef>, which is well suited for most scenarios.
For scenarios where you want conflict-free initialization of a shared inst, you should use <ActionLink action='os.downloadBotsAsInitialzationUpdate(bots, filename)'/>.
...
The downloaded bots will be stored in the Version 2 format of the <GlossaryRef term='aux'>AUX File Format</GlossaryRef>, which is better suited towards scenarios which require conflict-free initialization of a shared inst from the AUX file.
For an archive of the current state, you should use <ActionLink action='os.downloadBots(bots, filename)'/> which is better for scenarios which require direct access to the bot data.
...
```typescript title='Add a spherical grid that the user can see.'
os.addDropGrid({
    type: "sphere",
    position: { x: 0, y: 0, z: 3 },
    showGrid: true
...
If a position is not specified, then a position just below the current mouse position will be used. If on mobile, then the last touch position will be used or the center of the screen if the user has not touched the screen.
...
8. Focus the onClick tag in the systemPortal
```typescript
await os.focusOn(thisBot, {
    tag: 'onClick',
    portal: 'system'
...
9. Focus line 2 in the onClick tag in the sheetPortal
```typescript
await os.focusOn(thisBot, {
    tag: 'onClick',
    lineNumber: 2,
...
10. Focus index 9 through 15 in the onClick tag in the tagPortal
```typescript
await os.focusOn(thisBot, {
    tag: 'onClick',
    startIndex: 9,
...
6. Show an input for entering dates.
```typescript
...
7. Show an input with a list of options.
```typescript
...
8. Show an input with a list of checkboxes.
```typescript
...
9. Show an input with a dropdown of checkboxes.
```typescript
// Empty array means nothing is selected.
// To pre-select items, pass in an array with the indexes of the items you want selected.
const selectedItems = await os.showInput([], {
...
10. Show an input with a list of [radio buttons](https://en.wikipedia.org/wiki/Radio_button).
```typescript
// Null means nothing is selected.
// To pre-select an item, pass in the index of the item you want selected.
const selectedItem = await os.showInput(null, {
...
Shows a confirmation dialog using the given options.
...
```typescript title='Show a confirmation dialog'
...
```typescript title='Show a confirmation dialog with custom button text'
let confirmed = await os.showConfirm({
    title: 'Confirm',
...
### `os.showAlert(options)`
 
Shows an alert dialog using the given options.
Alert dialogs are useful for displaying important information to users that requires acknowledgment.
 
Returns a promise that resolves when the user dismisses the dialog by clicking the button or closing it.
...
#### Examples:
```typescript title='Show an alert dialog'
await os.showAlert({
    title: 'Notice',
...
```
```typescript title='Show an alert dialog with custom button text'
await os.showAlert({
    title: 'Warning',
...
os.toast('User acknowledged the warning');
```
...
2. Enable AR running at half the resolution of normal.
```typescript
os.enableAR({
...
2. Enable VR running at half the resolution of normal.
```typescript
os.enableVR({
    frameBufferScaleFactor: 0.5
});
...
<PossibleValuesTable>
  ...
  <PossibleValueCode value='miniMap'>
...
```typescript title='Find the bots that are directly to the right of (0,0,0) in the grid portal'
...
```typescript title='Find the bots that the mouse pointer is pointing at in the grid portal'
...
The **second parameter** is the 2D viewport position that the ray should start at. Viewport positions locate a specific point on the image that the camera produces.
...
```typescript title='Find the bots that are in the center of the screen'
const result = await os.raycastFromCamera("grid", new Vector2(0, 0));

os.toast('Found Bots: ' + result.botIntersections.map(b => b.id).join(', '));
```

```typescript title='Find the bots that are on the left-center edge of the screen'
...
```typescript title='Find the ray that passes through the center of the screen'
const ray = await os.calculateRayFromCamera("grid", new Vector2(0, 0));

os.toast('Calculated ray: ' + ray);
```

```typescript title='Find the ray that passes through the left-center edge of the screen'
...
Can be combined with <ActionLink action='math.intersectPlane(origin, direction, planeNormal?, planeOrigin?)'/> to find where on the ground the pointer is pointing.
...
Requests a wake lock that will keep the device screen awake. This will ask the user for permission to keep the screen awake.
...
```typescript title='Request a wake lock from the user.'
...
```typescript title='Disable the wake lock'
...
```typescript title='Get the current wake lock state'
...
Gets the list of updates that have occurred in the shared space.
Returns a promise that resolves with the list of updates.

Useful when combined with <ActionLink action='os.getInstStateFromUpdates(updates)'/> to track the history of an inst over time.

#### Examples

```typescript title='Get a list of updates to shared space'
const updates = await os.listInstUpdates();
```

### `os.getInstStateFromUpdates(updates)`

Calculates the inst state from the given list of updates.
...
```typescript title='Get the last 5 inst states in the shared space'
const updates = await os.listInstUpdates();
...
```typescript title='Calculate the last deltas from shared space updates'
const updates = await os.listInstUpdates();
...
Note that the inst update only contains bots and tag masks from the `shared` space.
Useful for saving the current shared state of the inst so that it can be restored later or transferred to another inst.

Unlike <ActionLink action='os.createInitializationUpdate(bots)'/>, this function creates an update that is linked to this inst.
...
```typescript title='Save the current inst state to a local bot'
...
```typescript title='Restore from a local bot'
...
Creates an inst update that, when applied, ensures the given bots are created on this inst.
Returns a promise that resolves with the inst update.

Note that you can apply the same update multiple times and you will end up with only one version of the bots saved in the update.
...
Unlike <ActionLink action='os.getCurrentInstUpdate()'/>, this function creates an update that is not linked to this inst. This means that applying the update to the inst it was created in will create duplicate bots.

The **first parameter** is the list of bots that should be included in the update.

#### Examples

```typescript title='Create an update with this bot and save it to a tag'
const update = await os.createInitializationUpdate([thisBot]);
tags.savedUpdate = update;
```

```typescript title='Create an update with all the bots in the home dimension'
...
Note that you can call this function with the same update multiple times and you will end up with only one version of the bots saved in the update.
Additionally, future changes to the bots will be preserved even if the update is applied again.
...
```typescript title='Apply an update that was saved to a tag'
...
```typescript title='Merge a list of updates'
...
Note that these rooms are unrelated to the meetPortal. The meetPortal is a preconfigured user interface that can be used to setup multimedia chat rooms, but is somewhat limited in its configurablilty.
...
Triggers the <TagLink tag='@onRoomJoined'/> listener once the room has been successfully joined.

Additionally, the following listeners will be triggered when the relevent events occur in the chat room:

-   <TagLink tag='@onRoomTrackSubscribed'/>
-   <TagLink tag='@onRoomTrackUnsubscribed'/>
-   <TagLink tag='@onRoomStreamLost'/>
-   <TagLink tag='@onRoomStreaming'/>
-   <TagLink tag='@onRoomSpeakersChanged'/>
-   <TagLink tag='@onRoomRemoteJoined'/>
-   <TagLink tag='@onRoomRemoteLeave'/>
-   <TagLink tag='@onRoomOptionsChanged'/>
...
The **second parameter** is optional and is the object that contains the additional parameters for joining the room.
...
```typescript title='Join the "myChat" room'
...
```typescript title='Join a room with the video stream disabled'
const result = await os.joinRoom("myChat", {
    video: false
});
if (result.success) {
...
Attempts to exit the given room using the given options.
Returns a promise that resolves with an object which indicates whether the operation was successful.

Triggers the <TagLink tag='@onRoomLeave'/> listener once the room has been left.

The **first parameter** is the name of the room that should be exited.

The **second parameter** is optional and is the object that contains the additional parameters for leaving the room.
It is an object with the following structure: 
...
```typescript title='Leave the "myChat" room'
...
Attempts to set the options for the given chat room. Useful for enabling/disabling video, audio, and screensharing.
...
Triggers the <TagLink tag='@onRoomOptionsChanged'/> and <TagLink tag='@onRoomTrackSubscribed'/>/<TagLink tag='@onRoomTrackUnsubscribed'/> listeners as needed for the specified changes.
...
```typescript title='Start screensharing'
...
```typescript title='Mute the microphone'
const result = await os.setRoomOptions("myChat", {
    audio: false
});
if (result.success) {
...
Attempts to get the current options for the given chat room. Useful for determining the current state of the local video (camera), audio (microphone), and screen streams.
...
```typescript title='Get the current options for a room'
...
Attempts to set the current options for the specified audio/video track in the specified room.
Returns a promise that resolves with an object that indicates whether the operation was successful.
...
The **second parameter** is the address of the audio/video track. Track addresses can be obtained via the <TagLink tag='@onRoomTrackSubscribed'/> listener.
...
```typescript title='Stop streaming a track'
...
```typescript title='Set the video quality on a track'
const result = await os.setRoomTrackOptions("myChat", "myTrack", {
    videoQuality: 'low'
});
if (result.success) {
...
```typescript title='Get the options for a track'
...
Attempts to get the current options for the specified remote user in the specified room.
...
The **second parameter** is the ID of the remote user whose options should be retrieved. Remote IDs can be obtained via the <TagLink tag='@onRoomRemoteJoined'/> listener.
...
Additionally, debuggers can be configured to be pausable. This allows you to set pause triggers (also known as [breakpoints](https://en.wikipedia.org/wiki/Breakpoint)) that temporarily stop the
...
```typescript title='Create a pausable debugger'
...
```typescript title='Create a normal debugger and copy this bot into it.'
...
Attaches the given debugger to the CasualOS frontend. This causes the given debugger to be treated like another inst that has been loaded simultaneously with the current inst.
This feature makes it useful for inspecting the bots in a debugger or even for setting up a sandbox that you control.

Note that because debuggers are entirely separate environments, the debugger gets its own configBot and portal bots. This means that in order for bots to show up in the portals, you need to set the corresponding portal on the debugger's configBot.
...
```typescript title='Create and attach a debugger'
const debug = await os.createDebugger();
...
```typescript title='Attach a debugger with a tag mapper that renames "home" tags to "testHome"'
const debug = await os.createDebugger();
...
Detaches the given debugger from the CasualOS frontend.
...
```typescript title='Detach a debugger'
const debug = await os.createDebugger();
...
The **first argument** is the function that should be called when the debugger is paused.
...
```typescript title='Listen for pauses on a debugger'
const debug = await os.createDebugger({
    pausable: true
});
...
Pause triggers can be used to tell the debugger where you want it to temporarily stop execution.
You specify the bot, tag, line and column numbers, and the debugger will stop before it executes the code at that location.
Additionally, the debugger will call all handlers that have been registered with <ActionLink action='onPause(handler)'/>.

The **first parameter** is the bot, bot ID, or trigger object that should be registered. If it is a bot or a bot ID, then a tag and options must be specified.
...
The **second parameter** is the name of the tag that the trigger should be set on.
It is ignored if a trigger object is specified for the first parameter.

The **third paramter** is the options that the trigger should use.
It is ignored if a trigger object is specified for the first parameter.
It should be an object with the following properties:
...
```typescript title='Set a pause trigger on a script'
const debug = await os.createDebugger({
    pausable: true
});
...
```typescript title='Update a pause trigger on a script'
const debug = await os.createDebugger({
    pausable: true
});
...
```typescript title='Remove a pause trigger'
const debug = await os.createDebugger({
    pausable: true
});
...
```typescript title='Enable a pause trigger'
const debug = await os.createDebugger({
    pausable: true
});
...
Disabled pause triggers will continue to be listed with <ActionLink action='listPauseTriggers()'/>, but will not cause a pause to happen while they are disabled.

The **first parameter** is the trigger or ID of the trigger that should be disabled.

#### Examples:

```typescript title='Disable a pause trigger'
const debug = await os.createDebugger({
    pausable: true
});
...
```typescript title='List the triggers that are set on this debugger'
const debug = await os.createDebugger({
    pausable: true
});
...
Gets a list of common trigger locations for the specified tag on the specified bot.
...
```typescript title='List common trigger locations for a script'
const debug = await os.createDebugger({
    pausable: true
});
...
```typescript title='Register a trigger from a common location'
const debug = await os.createDebugger({
    pausable: true
});
...
The **first parameter** is the debugger pause that was passed to the handler of <ActionLink action='onPause(handler)'/>.

#### Examples:

```typescript title='Resume execution on a debugger'
const debug = await os.createDebugger({
    pausable: true
});
...
This occurrs for common actions like <ActionLink action='os.toast(message, duration?)'/> and <ActionLink action='os.showInput(value?, options?)'/>.
...
```typescript title='Listen for actions to be enqueued in a debugger'
const debug = await os.createDebugger({
    pausable: true
});
...
The **first parameter** is the function that should be called when a tag is updated.
The first parameter of the function is an object that contains information about the update.

#### Examples:

```typescript title='Listen for tag updates in a debugger'
const debug = await os.createDebugger({
    pausable: true
});
...
The **first parameter** is the function that should be called when a tag mask is updated.
The first parameter of the function is an object that contains information about the update.
...
Additionally, these actions can only be automatically created for debuggers that are attached using <ActionLink action='os.attachDebugger(debug, options?)'/>.

Only available inside a debugger that was created using <ActionLink action='os.createDebugger(options?)'/>.

The **first parameter** is the function that should be called before a user action is performed.
...
This function works similarly to <ActionLink action='action.perform(action)'/> except that actions performed with it will also call handlers registered with <ActionLink action='onBeforeUserAction(handler)'/>.
...
Gets the current call stack for the debugger.
...
```typescript title='Get the call stack from a debugger'
...
```typescript title='Get the number of radians for a 90 degree angle'
...
```typescript title='Get the number of degrees for a Math.PI / 2 angle'
...
The **third parameter** is optional and is the normal vector that the plane should use. For 2D planes, the normal vector is the 3D direction that is perpendicular to the the surface of the plane.
...
Creates a mod that sets the <TagLink tag='cameraPositionOffset'>cameraPositionOffset</TagLink> tags to the given 3D position.
...
Creates a mod that sets the <TagLink tag='cameraRotationOffset'>cameraRotationOffset</TagLink> tags to the given 3D position.
...
-   `sha256` (equivalent to <ActionLink action='crypto.sha256(...data)'/>)
-   `sha512` (equivalent to <ActionLink action='crypto.sha512(...data)'/>)
...
```typescript title='Calculate the SHA-256 hex hash of a string'
const hash = crypto.hash("sha256", "hex", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the SHA-256 base64 hash of a string'
const hash = crypto.hash("sha256", "base64", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the SHA-256 raw hash of a string'
const hash = crypto.hash("sha256", "raw", "hello, world");
console.log(hash);
```

```typescript title='Calculate the SHA-512 hex hash of a string'
const hash = crypto.hash("sha512", "hex", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the SHA-1 hex hash of a string'
...
-   `hmac-sha256` (equivalent to <ActionLink action='crypto.hmacSha256(key, ...data)'/>)
-   `hmac-sha512` (equivalent to <ActionLink action='crypto.hmacSha512(key, ...data)'/>)
...
```typescript title='Calculate the hexadecimal HMAC-SHA256 of a string with a key'
const hash = crypto.hmac("hmac-sha256", "hex", "key", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the Base64 HMAC-SHA256 of a string with a key'
const hash = crypto.hmac("hmac-sha256", "base64", "key", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the raw HMAC-SHA256 of a string with a key'
const hash = crypto.hmac("hmac-sha256", "raw", "key", "hello, world");
console.log(hash);
```

```typescript title='Calculate the hexadecimal HMAC-SHA512 of a string with a key'
const hash = crypto.hmac("hmac-sha512", "hex", "key", "hello, world");
os.toast(hash);
```

```typescript title='Calculate the hexadecimal HMAC-SHA1 of a string with a key'
...
The **first parameter** is the [Uint8Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) of bytes that should be formatted into Base64.

#### Examples:

```typescript title='Format a byte array into Base64'
...
Returns the new [Uint8Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) array.

The **first parameter** is the Base64 formatted string that should be converted to a [Uint8Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) byte array.

#### Examples:

```typescript title='Convert a Base64 string into bytes'
...
The **first parameter** is the [Uint8Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) of bytes that should be formatted into hexadecimal.

#### Examples:

```typescript title='Format a byte array into hexadecimal'
os.toast(bytes.toHexString(new Uint8Array([ 255, 254, 253 ]))); // fffefd in bytes
```

### `bytes.fromHexString(hex)`

Converts the given [hexadecimal](https://en.wikipedia.org/wiki/Hexadecimal) string into a byte array that contains the data represented by the string.
Returns the new [Uint8Array](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) array.
...
```typescript title='Convert a hex string into bytes'
...
```tsx title='Setup an app with an input box'
...
```tsx title='Display a header'
os.compileApp('myApp', <h1>Hello World!</h1>);
```

```tsx title='Display a button'
os.compileApp('myApp', 
    <button onClick={ () => os.toast("You clicked the button!")}>
        Click Me!
    </button>
...
```tsx title='Display an input box'
os.compileApp('myApp', 
    <input onInput={ (e) => { tags.label = e.target.value } } />
);
```

```tsx title='Display a slider input'
os.compileApp('myApp', 
...
An object that contains [hooks](https://preactjs.com/guide/v10/hooks) that make programming custom apps easier.
See the [preact documentation](https://preactjs.com/guide/v10/hooks) for more information.

#### Examples

```tsx title='Create a counter'
...
```tsx title='Render an app to the document without using os.compileApp()'
...
Includes tags like <TagLink tag='color'/> and <TagLink tag='gridPortal'/>, but not user-defined ones like <TagLink tag='[dimension]'/>.

#### Examples

```typescript title='Get the list of built-in tags'
...
4. Register a prefix with a name.
```typescript
await os.registerTagPrefix("🔺", {
    language: "jsx"
    name: 'Triangle'
...
Analytics actions are actions that record events to [Simple Analytics](https://www.simpleanalytics.com/).

### `analytics.recordEvent(name, metadata?)`

Records the given event to Simple Analytics using their [Events API](https://docs.simpleanalytics.com/events).
Returns a promise that resolves when the event has been recorded.

The **first parameter** is the name of the event that should be recorded. Must consist of [only alphanumeric characters and underscores](https://docs.simpleanalytics.com/events#valid-event-names).

The **second parameter** is optional and should be an object containing the metadata that should be collected with the event. (See [metadata](https://docs.simpleanalytics.com/metadata) for more information)

#### Examples

```typescript title='Record an analytics event'
await analytics.recordEvent('my_event');
```

```typescript title='Record an analytics event with some metadata'
await analytics.recordEvent('my_event', {
    myNumber: 1
});
```
