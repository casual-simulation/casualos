...
Creates a bot filter which includes bots that were created by the given bot. That is, they have <TagLink tag='aux.creator' /> set to the <TagLink tag='id' /> of the given bot.

The **first parameter** is the bot that created the other bots.

<Alert type='info'>
    This function behaves exactly like <NormalCode>byTag("aux.creator", getTag(bot, "#id"))</NormalCode>.
</Alert>
...
server.setupChannel("test", {
    "onCreate()": `
        create(null, {
            "aux.context": "abc",
            "aux.context.visible": true,
        });

        create(null, {
            "aux.label": "This is in the context!",
            "aux.color": "blue",
            "abc": true
        });
