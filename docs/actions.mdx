...
Creates a new bot or combination of bots with the given <TagLink tag='auxCreator'/> and mods. Also triggers <TagLink tag='onCreate()'/> on all the created bots.

The **first parameter** is the bot or <TagLink tag='id'/> that should be used as the <TagLink tag='auxCreator'/> for the new bot(s).
If you specify `null`, then the new bot(s) will not have a <TagLink tag='auxCreator'/>.
...
Creates a new temporary bot or combination of temporary bots with the given <TagLink tag='auxCreator'/> and mods. Also triggers <TagLink tag='onCreate()'/> on all the created bots.
...
<Alert type='info'>
    This function behaves exactly like <NormalCode>byTag("auxCreator", getTag(bot, "#id"))</NormalCode>.
</Alert>
...
```typescript
let bots = getBots(createdBy(this));
```
