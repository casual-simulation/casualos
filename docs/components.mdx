...
export const TagLink = ({tag}) => tag.endsWith('()') ? 
  (<ListenTagLink tag={tag}/>) : 
  (<DataTagLink tag={tag}/>);

export const DataTagLink = ({tag}) => (
  <a href={withBaseUrl('docs/tags') + '#' + tag.replace(/\./g, '').toLowerCase()}>
    <NormalCode>#{tag}</NormalCode>
  </a>
);

export const ListenTagLink = ({tag}) => (
  <a href={withBaseUrl('docs/listen-tags') + '#' + tag.replace(/[\.\(\)]/g, '').toLowerCase()}>
    <NormalCode>#{tag}</NormalCode>
  </a>
);

export const ActionLink = ({action}) => (
  <a href={withBaseUrl('docs/actions') + '#' + action.replace(/[\.\(\)\,]/g, '').replace(/\s/g, '-').toLowerCase()}>
...
export const Badge = ({type, children}) => (<span class={`badge badge--${type}`}>{children}</span>);
...
export const FunctionCode = ({name}) => {
  const func = source.getFunction(name);
  return (
    <pre>
      <code className='typescript'>
        {func ? func.getText() : 'CODE NOT AVAILABLE'}
      </code>
    </pre>
  );
};
