...
export const AnyCursorValues = ({}) => (<>
  <PossibleValueCode value='auto'>
      ...
      For bot portal bots, this means that the arrow cursor is used.
...
export const BotPortalBadge = ({url}) => (<Badge type='normal' title='This tag is available on the bot portal bot.'>botPortalBot</Badge>);
