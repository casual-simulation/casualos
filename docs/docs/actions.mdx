...
<FunctionCode name='subtractMods'/>
...
<FunctionCode name='getBotTagValues'/>

Gets a list of all the values in the universe for the given tag. Optionally accepts a filter for the tag values.
...
1. Send a HTTP GET request to `https://example.com`.
```typescript
webhook({
    method: 'GET',
...
1. Send a HTTP POST request to `https://example.com`.
```typescript
...
1. Find all the bots with `#height` set to `1` and `#auxColor` set to `red`.
```typescript
...
The **third parameter** is the neighboring direction to check.
...
<FunctionCode name='superShout' />

Sends a shout to all of the other universes that are loaded.
...
1. Send a `hello` super shout to all the loaded universes.
```typescript
superShout("hello");
```
...
However, before any action is performed, it is run through the <TagLink tag='@onUniverseAction'/> listener which can decide whether reject an action using <ActionLink action='action.reject(action)'/>.
...
Instead it is wrapped as a `device` action and sent to <TagLink tag='@onUniverseAction'/> for processing. This lets you decide whether to allow players to send messages to each other and what the effect of those messages are.
...
<FunctionCode name='reject'/>

Prevents a previous action from being performed.

This is especially useful when used in a <TagLink tag='@onUniverseAction'/> listener since it lets you reject actions before they are performed.
...
<FunctionCode name='checkout'/>

Shows a payment screen modal that lets the player purchase something.
Triggers the <TagLink tag='@onCheckout'/> shout on the server inside the given processing universe.
...
<FunctionCode name='showJoinCode'/>

Shows a [QR Code](https://en.wikipedia.org/wiki/QR_code) with a link to join the given universe and dimension.
If the universe and dimension are omitted, then the current universe and dimension will be used.

The **first parameter** is the universe that the code should be shown for.
...
2. Show a join QR Code for a universe and dimension.
```typescript
player.showJoinCode("universe", "dimension");
```
...
### `player.loadUniverse(universe)`

<FunctionCode name='loadUniverse'/>

Loads the given univese into the current browser tab. When the universe is loaded, the <TagLink tag='@onUniverseSubscribed'/> shout will be triggered.

Note that separate universes cannot interact directly. They must instead interact via super shouts.

The **first parameter** is the name of the universe to load.

#### Examples:

1. Load the `fun` universe.
```typescript
player.loadUniverse("fun");
```

### `player.unloadUniverse()`

Unloads the given universe from the current browser tab. When the universe is unloaded, the <TagLink tag='@onUniverseUnsubscribed'/> shout will be triggered.

The **first parameter** is the name of the universe to unload.

#### Examples:

1. Unload the `fun` universe.
```typescript
player.unloadUniverse('fun');
```
...
<FunctionCode name='hasBotInInventory'/>

Determines if the given bot is in the dimension that is currently loaded into the <TagLink tag='auxInventoryPortal'/> portal.
...
<FunctionCode name='goToDimension'/>

Loads the given dimension into the <TagLink tag='auxPagePortal'/> portal.
...
<FunctionCode name='openDevConsole'/>
...
<FunctionCode name='isInDimension'/>

Gets whether the given dimension is loaded into the <TagLink tag='auxPagePortal'/> portal.
...
<FunctionCode name='backupToGithub'/>

Saves all the universes to GitHub as a private Gist using the given authentication token.
...
<Alert type='info'>
    By default, backing up to GitHub is disabled on a universe.
    To enable it, you have to write a custom <TagLink tag='@onUniverseAction'/> listener on the config bot and run an <ActionLink action='action.perform(action)'/>.
</Alert>

#### Examples:

1. Backup all the universes to a Github Gist.
```typescript
...
<FunctionCode name='backupAsDownload'/>

Saves all the universes to a zip file and downloads it to the session matching the given target.

The **first parameter** is an object that indicates which session the zip file should be downloaded to.

<Alert type='info'>
    By default, backing up as a download is disabled on a universe.
    To enable it, you have to write a custom <TagLink tag='@onUniverseAction'/> listener on the config bot and run an <ActionLink action='action.perform(action)'/>.
</Alert>
...
<Alert type='info'>
    By default, shell scripts are disabled on a universe.
    To enable them, you have to write a custom <TagLink tag='@onUniverseAction'/> listener on the config bot and run an <ActionLink action='action.perform(action)'/>.
</Alert>

### `server.loadFile(path, options?)`

<FunctionCode name='serverLoadFile'/>
...
<Alert type='info'>
    By default, loading files is disabled on a universe.
    To enable it, you have to write a custom <TagLink tag='@onUniverseAction'/> listener on the config bot and run an <ActionLink action='action.perform(action)'/>.
</Alert>
...
<FunctionCode name='serverSaveFile'/>
...
<Alert type='info'>
    By default, saving files is disabled on a universe.
    To enable it, you have to write a custom <TagLink tag='@onUniverseAction'/> listener on the config bot and run an <ActionLink action='action.perform(action)'/>.
</Alert>
...
1. Set a bot's <TagLink tag='auxColor'/> to `green`.
```typescript
...
1. Get the <TagLink tag='auxColor'/> tag from `this` bot.
```typescript
const color = getTag(this, "#auxColor");
```
