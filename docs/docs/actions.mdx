...
### `server.configureGpio(pin, mode)`


This configures a GPIO pin on the physical server so that you can access them as inputs or outputs.
The action is only executed if <TagLink tag='@onStoryAction'/> has been configured to perform events in `device` actions.

Returns a promise that resolves when the pin has been configured.

The **first parameter** is the BCM Pin on the physical server.

The **second parameter** is 'in' or 'out' to set the pin as an input pin or an output pin.

#### Examples:

1. Configure Pin 17 as output.
```typescript
server.configureGpio(17, "out")
```

### `server.setGpio(pin, value)`


This sets a GPIO pin on the physical server to either HIGH (1) or LOW (0).
The action is only executed if <TagLink tag='@onStoryAction'/> has been configured to perform events in `device` actions.
...
The **second parameter** is '0' or '1' to set the pin as an HIGH or LOW.
...
This reads a given GPIO Pin and returns the state/value of the pin.
The action is only executed if <TagLink tag='@onStoryAction'/> has been configured to perform events in `device` actions.
...
2. Configure Pin 17 as output and initialize the pin as "LOW".
```typescript
rpioOpen(17, "OUTPUT", "LOW")
```
...
1. Set Pin 17 to HIGH.
```typescript
server.rpioWrite(17, "HIGH")
```
...
[Encrypts](https://en.wikipedia.org/wiki/Encryption) the given data using the given password and returns the result as a promise.
...
```typescript
const encrypted = await crypto.encrypt("key", "hello, world");
player.toast(encrypted);
```

### `crypto.decrypt(password, data)`

[Decrypts](https://en.wikipedia.org/wiki/Encryption) the given data using the given password and returns the result as a promise.
...
```typescript
const decrypted = await crypto.decrypt("key", "v1.vWUhsdfiKkxXi9Rt+BBNbcP/TiHZpxUL.iikPvWN6rNncY3j045gM0268MoRi0NNf.IpWYgzXQmjRea4MNLDXB1GmrinWLSSOMw+NfqeE=");
