...
### `sendRemoteData(playerId, name, arg?)`

Sends a <TagLink tag='@onRemoteData'/> shout to the player with the given ID or players if given a list of IDs.
...
// The other player will receive a @onRemoteData with 
// that.name === "custom" and that.that === "Hello"
sendRemoteData(otherPlayerBotId, "custom", "Hello");
```
...
// All other players will receive a @onRemoteData with
// that.name === "custom" and that.that === "Hello"
sendRemoteData(otherPlayers, "custom", "Hello");
```

### `remoteWhisper(playerId, name, arg?)`

NOTICE: This method is deprecated in favor of <ActionLink action='sendRemoteData(playerId, name, arg?)'/>.
