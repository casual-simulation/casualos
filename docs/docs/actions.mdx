...
**close_on_exit** can be `true` or `false`.
...
This method is provided to allow explicit shutdown when using `close_on_exit: false` and a custom exit handler.
The action is only executed if <TagLink tag='@onStoryAction'/> has been configured to perform events in `device` actions.
...
1. Initialize rpio with `close_on_exit: false` and manually exiting.
```typescript
server.rpioInit({close_on_exit: false});
...
### `server.rpioReadBuf(pin, buffer, length)`
...
The **second parameter** is the buffer to read.

The **third parameter** is the length of the buffer.

#### Examples:

1. Read the value of Pin 16 10,000 times in a row, storing the values in buf
```typescript
var buf = new Buffer(10000);
server.readbuf(16, buf);
```
...
### `server.rpioWriteBuf(pin, buffer, length)`
...
The **second parameter** is the buffer to write to.
