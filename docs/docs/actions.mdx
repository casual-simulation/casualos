...
For bots in the page or inventory portals, this animates the camera such that the portal focus point is placed on the given bot or position.
...
For the page and inventory portals this is when the animation finishes and rejects if the user takes control of the camera during the animation.
...
let options: {
    ...
    */
     ...
     * If not specified, then the bot will be focused in all the portals it is in. (page, inventory and menu)
