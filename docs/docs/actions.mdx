...
Note that the bots don't load immediately. They will automatically be added to the server state once they have been loaded.
...
1. Destroy all the error bots in this server:
```typescript
server.destroyErrors();
```
