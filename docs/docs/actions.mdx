...
Causes the circular wipe animation to open around the screen. This can be used to reveal the bot portal after transitioning between screens.
...
1. Reveal the bot portal with a circular wipe animation.
```typescript
await os.openCircleWipe();
os.toast("Revealed!");
```

2. Hide the bot portal and show it after an additional second.
```typescript
...
Causes a circular wipe animation to close around the screen. This can be used to cover the bot portal while transitioning between scenes.
Returns a promise that resolves when the animation has finished running.
...
1. Hide the bot portal with a circular wipe animation.
```typescript
await os.closeCircleWipe();
os.toast("Hidden!");
```
...
For bots in the bot or mini portals, this animates the camera such that the portal focus point is placed on the given bot or position.
...
For the bot and mini portals this is when the animation finishes and rejects if the user takes control of the camera during the animation.
...
let options: {
    ...
    * For the bot and mini portals, possible values are between `0` and `80`. `1` is the default.
...
Enables Point-of-View mode on the device. Useful for getting a "ground level" view in the bot portal.
...
Gets the dimension that is loaded into the <TagLink tag='botPortal'/> portal.

<Alert type='info'>
    This function behaves exactly like <ActionLink action='os.getPortalDimension(portal)'/> when given "botPortal".
</Alert>

#### Examples:

1. Show a message of the dimension that is currently in the <TagLink tag='botPortal'/> portal.
```typescript
...
<PossibleValuesTable>
  <PossibleValueCode value='bot'>
    The <TagLink tag='botPortal'/>.
  </PossibleValueCode>
...
1. Get the dimension that is currently showing in the <TagLink tag='botPortal'/>.
```typescript
const dimension = os.getPortalDimension('bot');
```
...
<PossibleValuesTable>
  <PossibleValueCode value='bot'>
    The camera position in the bot portal. (Default)
  </PossibleValueCode>
  <PossibleValueCode value='mini'>
...
1. Get the position of the player in the bot portal.
```typescript
const position = os.getCameraPosition("bot");
```
...
<PossibleValuesTable>
  <PossibleValueCode value='bot'>
    The camera rotation in the bot portal. (Default)
  </PossibleValueCode>
  <PossibleValueCode value='mini'>
...
1. Get the rotation of the player in the bot portal.
```typescript
const rotation = os.getCameraRotation("bot");
```
...
<PossibleValuesTable>
  <PossibleValueCode value='bot'>
    The camera focus point in the bot portal. (Default)
  </PossibleValueCode>
  <PossibleValueCode value='mini'>
...
1. Get the focus point of the player in the bot portal.
```typescript
const focusPoint = os.getFocusPoint("bot");
```
...
Loads the given dimension into the <TagLink tag='botPortal'/> portal.
Triggers the <TagLink tag='@onPortalChanged'/> shout for the `botPortal`.
...
Gets whether the given dimension is loaded into the <TagLink tag='botPortal'/> portal.
...
Calculates the 3D point that a ray starting at the given origin point and traveling in the given direction intersects the bot portal ground plane.
...
Note that the tween will only work if the given dimension is currently in the bot portal or mini portal.
