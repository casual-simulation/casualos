...
Portal actions are a way to register and display customized portals inside CasualOS.
They work in tandem with listen tags and bots to create arbitrarily complex experiences.

### `portal.register(portalId, bot, options)`

Registers a portal with the given ID, bot, and options. Returns a promise that resolves when the portal has been registered.
Can be called multiple times with new options to override previous options.

Once setup, CasualOS will send a <TagLink tag='@onPortalSetup'/> whisper to the given bot.
At this point, you can call <ActionLink action='portal.reset(portalId, content)'/> to set what the portal displays.

CasualOS will also define a global variable `{portalId}Bot` that points to the given bot.

The **first parameter** is the ID that the portal should have.

The **second parameter** is the bot that should represent the portal. This is the bot that recieves the <TagLink tag='@onPortalSetup'/> whisper and
should generally be in charge of calling <ActionLink action='portal.reset(portalId, content)'/>.

The **third parameter** is an object that specifies options about the portal. It should have the following structure:
...
Currently, portals can only have one type: `html`. In the future, we'll add more types to facilitate different experience types.

`html` means that <ActionLink action='portal.reset(portalId, content)'/> expects to recieve HTML objects.
...
```typescript title='Setup a basic portal'
await portal.register('basicPortal', thisBot, {
    type: 'html',
});

portal.reset('basicPortal', html`
    <h1>Hello World!</h1>
`);
```

```typescript title='Setup a portal with a button'
await portal.register('buttonPortal', thisBot, {
    type: 'html',
});

portal.reset('buttonPortal', html`
...
```typescript title='Setup a portal with an input box'
await portal.register('inputPortal', thisBot, {
    type: 'html',
});

portal.reset('inputPortal', html`
    <input onInput=${ (e) =>  tags.label = e.target.value }>
`);
```

### `portal.reset(portalId, content)`

Resets the portal with the given ID to display the given content.
Each time this function is called, the portal will be cleared and will display the specified content.

Used in tandem with <ActionLink action='portal.register(portalId, bot, options)'/> to create custom portals.

The **first parameter** is the ID of the portal.

The **second parameter** is the content that the portal should display.

#### Examples:

```typescript title='Display a header'
portal.reset('myPortal', html`
    <h1>Hello World!</h1>
`);
```
...
```
@await portal.registerPrefix('📖');
await customPortal.open('my-portal', null, '📖main');
```
...
### `customPortal.open(portalId, bot, tag?, options?)`
...
await customPortal.open("pagePortal", myConfigBot);
```

2. Open a custom portal for the `test` tag.
```typescript
await customPortal.open("my-portal", null, "test");
```
...
```typescript
await customPortal.open("my-portal", null, `console.log("Hello, Portal!");`, {
    mode: "source"
});
```

### `customPortal.buildBundle(tag)`
...
```typescript
const bundle = await customPortal.buildBundle("test");
```
