...
Use <ActionLink action='crypto.asymmetric.encrypt(keypair, data)'/> and <ActionLink action='crypto.asymmetric.decrypt(keypair, signature, data)'/> to encrypt and decrypt the data.
