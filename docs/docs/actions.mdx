...
Before `server.setupChannel()` will work, you need to setup the <TagLink tag='@onUniverseAction'/> tag to execute events from remote devices.
This is because when you run `server.setupChannel()` from a PC or phone (a.k.a. A "remote device"), it needs to send an event to the server to actually create the new universe.
