...
Creates a bot filter that includes bots that are in the given context. That is, they have the given tag set to `true`.
...
Creates a bot filter that includes bots that are in the given context and at the given X and Y position.
...
Creates a bot filter that includes bots in the same stack as the given bot. The given bot will always be included by this filter as long the given bot is in the given context.
...
Sends an action to the server that will create the given channel if it does not exist and places a clone of the given bot or mod in it.
...
The following script will execute `setup_channel` events that were sent from other devices:
```typescript
