...
Vector2 is a class that makes it easy to represent and manipulate 2D points and directions. In math, a vector is simply a list of points, so Vector2 represents 2 points.
For example, you can use Vector2 objects with the <TagLink tag='[dimension]Position'/> tag to easily set both the X and Y coordinates of a bot in a single tag.

When you save a Vector2 to a tag, CasualOS will automatically format it so that it is automatically detectable as a vector and will always be represented the same no matter where in the world you are.

For example, saving `➡️1,2` to the `vector` tag will make `tags.vector` a Vector2 value that represents the point `(x: 1, y: 2)`.
...
```typescript
os.tip("This is in the center of the screen.", configBot.pixelWidth / 2, configBot.pixelHeight / 2);
```
