...
4. Snap the dragged bot to the center or bot faces.
```typescript
os.addDropSnap({
    position: {
        x: 0,
...
The **first parameter** is the bot, bot ID, or position that the player's view should be moved to. If null, then the current focus animation will be canceled.
...
let options: {
     ...
     * 
     * If the computer does not have a video device attached (like a web cam), then setting this to true
     * will cause an error.
     * 
     * Defaults to true.
     */
