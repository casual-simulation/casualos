...
const causalTreeCLI = path.resolve(src, 'causal-tree-cli');
const bin = path.resolve(causalTreeCLI, 'bin');
const packageJson = path.resolve(causalTreeCLI, 'package.json');
...
function createConfigs(dev, version) {
    ...
    return [
        [
            'Proxy',
            {
                entryPoints: [path.resolve(causalTreeCLI, 'index.ts')],
                outfile: path.resolve(bin, 'causal-tree-cli.js'),
                platform: 'node',
                target: ['node14.16'],
                external: externals,
                define: {
