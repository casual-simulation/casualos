...
const casualosInfra = path.resolve(src, 'casualos-infra');
const dist = path.resolve(casualosInfra, 'dist');
const outputFile = path.resolve(dist, 'cli.js');
const packageJson = path.resolve(casualosInfra, 'package.json');
...
export function createConfigs(dev, version) {
    ...
    return [
        [
            ...
            {
                entryPoints: [
                    path.resolve(casualosInfra, 'index.ts'),
                    path.resolve(casualosInfra, 'cli.ts'),
                ],
                outdir: dist,
                platform: 'node',
                target: ['node18.18'],
