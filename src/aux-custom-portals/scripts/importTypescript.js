...
const TYPESCRIPT_LIB_SOURCE = path.join(
    __dirname,
    '../../../node_modules/@casual-simulation/typescript/lib'
);
const TYPESCRIPT_LIB_DESTINATION = path.join(
    __dirname,
    '../monaco/typescript/lib'
);
...
(function () {
    ...
    const npmLsOutput = JSON.parse(
        child_process
            .execSync(
                'npm ls @casual-simulation/typescript --depth=0 --json=true',
                { cwd: path.resolve(__dirname, '../../..') }
            )
            .toString()
    );
    const typeScriptDependencyVersion =
        npmLsOutput.dependencies['@casual-simulation/typescript'].version;
...
export const typescriptVersion = "${typeScriptDependencyVersion}";\n`
    ...
    const runtimeRequires =
        ...
        linesWithRequire.filter(
            (l) =>
                !l.includes(': diag(') && !l.includes('ts.DiagnosticCategory')
        );
    ...
    if (runtimeRequires && runtimeRequires.length && linesWithRequire) {
        ...
        console.error(
            runtimeRequires
                .map((r) => `${r} (${tsServicesNoCommentedRequire.indexOf(r)})`)
                .join('\n')
        );
        process.exit(1);
    }
...
`;
    ...
    let dtsServices = fs
        .readFileSync(
            path.join(TYPESCRIPT_LIB_SOURCE, 'typescriptServices.d.ts')
        )
        .toString();
    dtsServices += `
// MONACOCHANGE
