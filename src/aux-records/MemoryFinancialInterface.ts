...
/**
 * ! MemoryFinancialInterface.ts is deprecated, its initial purpose was to provide a memory (local) implementation of the FinancialInterface.
 * ! However, to actually create a local memory implementation, we need to emulate the entire TigerBeetle API, which is not feasible.
 * ! The reason a local memory implementation would need to emulate TigerBeetle, is because TigerBeetle's financial model is the base of the FinancialInterface.
 * ! Unlike other memory implementations, this one is not a simple in-memory store, it is a full emulation of the TigerBeetle API and its behavior.
 * ! This is not a quick/easy task and requires larger amounts of attention (than what is allocable currently) to implement correctly as well as to maintain (match with new updates).
 * ! The MemoryFinancialInterface.ts file is kept for reference.
 */
...
//     ): CreateTransferError {
//         if (this._transfers.find((t) => t.id === transfer.id)) {
