...
export class TypeScriptWorker
    ...
    private _scriptPrefixes: string[] = [];
    ...
    addScriptPrefixes(prefixes: string[]) {
        this._scriptPrefixes.push(...prefixes);
    }
    ...
    async getSyntacticDiagnostics(fileName: string): Promise<Diagnostic[]> {
        ...
        const diagnostics = this._languageService.getSyntacticDiagnostics(
            fileName
        );
    ...
    async getSemanticDiagnostics(fileName: string): Promise<Diagnostic[]> {
        ...
        const diagnostics = this._languageService.getSemanticDiagnostics(
            fileName
        );
        return TypeScriptWorker.clearFiles(diagnostics);
    }
...
export interface ICreateData {
    ...
    casualOSModuleResolution?: boolean;
}
