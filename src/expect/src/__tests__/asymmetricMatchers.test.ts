    ...
    [
        ...
        [
            ...
            (function () {
                ...
                Object.defineProperty($someGeneratorFunc, 'name', {
                    value: '',
                });
...
test('ObjectContaining matches', () => {
    [
        ...
        objectContaining({ foo: 'foo' }).asymmetricMatch({
            foo: 'foo',
            jest: 'jest',
        }),
...
test('ObjectNotContaining inverts ObjectContaining', () => {
    [
        ...
        jestExpect(
            objectNotContaining(sample).asymmetricMatch(received)
        ).toEqual(!objectContaining(sample).asymmetricMatch(received));
    });
});
...
test('StringNotContaining matches string against string', () => {
    jestExpect(stringNotContaining('en*').asymmetricMatch('queen*')).toBe(
        false
    );
