...
describe('getObjectSubset', () => {
    [
        ...
        test(
            `expect(getObjectSubset(${stringify(object)}, ${stringify(
                subset
    ...
    describe('calculating subsets of objects with circular references', () => {
        test('simple circular references', () => {
            ...
            expect(getObjectSubset(circularObjB, circularObjA)).toEqual(
                circularObjB
            );
        ...
        test('transitive circular references', () => {
            ...
            transitiveCircularObjA.nestedObj = {
                parentObj: transitiveCircularObjA,
            };
            ...
            expect(
                getObjectSubset(nonCircularRef, transitiveCircularObjA)
            ).toEqual({
                nestedObj: { otherProp: {} },
            });
        });
...
describe('subsetEquality()', () => {
    ...
    describe('matching subsets with circular references', () => {
        test('simple circular references', () => {
            ...
            expect(subsetEquality(primitiveInsteadOfRef, circularObjA1)).toBe(
                false
            );
        });
...
describe('arrayBufferEquality', () => {
    test('returns undefined if given a non instance of ArrayBuffer', () => {
        ...
        expect(
            arrayBufferEquality(new Date(), new ArrayBuffer(2))
        ).toBeUndefined();
    });
